...
| 3.6 | **A `[Route]` on any controller but the base.** `BaseController` carries `[Route("api/[controller]")]` and no other controller declares one — the URL prefix is decided in a single file. `Find:` `grep -rn "\[Route(" --include=*.cs src/Web/Controllers/` and discard the base controller's own line; **every remaining hit is the finding.** This is the check that makes a separate controller for a nested route family impossible, because such a controller cannot exist without declaring its own route — so 3.5's two-module name and this are usually the same defect seen from two sides, and either one alone is enough to report it. **A file already in the tree is a hit like any other**: cited as *"the pattern we already use"*, it is a rule violation being quoted as a rule · `api-surface`, *Routes* | **HIGH** |
