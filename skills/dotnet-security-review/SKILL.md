...
| 3.3 | **A weak or hand-rolled cryptographic primitive.** `Find:` `grep -rn "MD5\|SHA1\|DESCryptoServiceProvider\|CipherMode.ECB\|new Random(\|NewId\.\|Guid.NewGuid\|Ulid\|Comb" src/`. Flag only where the value is a *security* value — a password, token, signature, nonce, or a value handed out as a one-time credential such as a recovery or activation code. **Identifier generators belong on this list exactly there**: `NewId`, `Guid.NewGuid()`, COMB and ULID generators produce *unique* values, not *unpredictable* ones — sequential or timestamp-led by construction — and a credential only has to be guessed once; uniqueness protects against nothing an attacker wants. **A weak hash used as a cache key, an ETag or a shard selector is not a finding, and an identifier generator minting an ordinary id is not one either**; say which you found before writing it up. Where the value comes off the house shared non-cryptographic `Random`, the finding is the **call site** routing through it a value that must stay recoverable — a signing key, a client secret, a long-lived API key — never the generator itself, which is canon; a generated value hashed before storage is the sanctioned case. The house prescribes no cryptography beyond the JWT layer, so name the exposure and ask for the algorithm decision rather than dictating one · universal, reinforced by `common-extensions`, `references/anti-patterns.md`, *13. A credential drawn from the shared non-cryptographic `Random`* | **HIGH** |
| 3.4 | **A path or process argument built from request data.** `Find:` `grep -rn "Path.Combine\|File.Open\|File.Read\|File.Write\|new FileStream\|Process.Start" src/` and read each argument back to its source. A request-supplied filename containing `..` escapes the intended directory, and `Path.Combine` discards everything before a rooted argument — `/etc/x` silently becomes the whole path. The same shape reaching a process argument is command execution. A caller-supplied filename reaching an object-storage key is the same shape one layer out, and the house answer there is the single key generator — generated tick prefix, sanitized stem — not a sanitize written at the call site. The fix is a generated name plus a resolved-path containment check, not a blacklist of `..` · universal, reinforced by `file-storage`, *One key generator, one format* | **HIGH; CRITICAL when the path is read or written without further checks** |
...
| 4.2 | **Several permissions on one attribute read as a conjunction.** `Find:` `grep -rn "\[HasPermission(" src/Web/` and flag every call listing two or more permission codes. The handler calls `HasAnyPermission…`: **several codes mean ANY, not ALL.** An author writing two codes to mean "both" has authorized strictly more than intended, it compiles, and it tests green for anyone holding either. A conjunction is not expressible within one attribute — it is written as **stacked attributes**: two `[HasPermission]` on one action are two policies and the framework passes the action only when every policy passes, so stacked means ALL. **Flag the reverse edit too**: merging two stacked attributes into one reads as cleanup and silently flips ALL into ANY, widening access · `auth-and-security`, `references/permission-internals.md` §3 | **HIGH** |
...
```markdown
# Security review: <scope> — <one line on what it covers>
...
| **Method** | <which layers ran; and that every CRITICAL/HIGH was re-checked at the cited `file:line`> |
...
- **Defect:** <what is exposed>
- **Failure:** <the reach — unauthenticated / any caller / another family / admin — and what it yields>
- **Fix:** <the change that closes it> · <owning skill>
...
| Credentials and their settings files in any infrastructure topic — cache, search, object storage, outbound HTTP | `distributed-caching`, `elasticsearch-search`, `file-storage`, `http-client-factory` |
