...
3. **Test, only on a green build with the fixture up.** `dotnet test <csproj>
   ...
   attributable. **Never add a parallelism flag.** How much of this tier runs at
   once is decided by its fixture attachment and its isolation strategy — a
   collection fixture with a reset serializes on purpose, because the classes
   share one database and parallel resets delete each other's rows. Overriding
   that from the command line produces failures the code does not have. Slowness
   here is a design decision, not a knob you are holding.
...
| "Running with more parallelism would be faster" | The suite's own fixture and isolation strategy set that; where a reset is in play the classes share one database and serialize on purpose. Forcing it apart produces failures the code does not have. |
| "The same tests fail differently each run — I'll report the last one" | Report this run's facts, and report them as this run's: identical counts are not implied. Whether the tier is non-deterministic is decided across runs, which is the flow's comparison to make — but a run that contradicts an earlier one is evidence, not noise to smooth over. |
