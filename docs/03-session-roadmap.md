...
| post-rubrics | ~~`dotnet-test-report` hook~~ | **Shipped at 0.3.44 as `hooks/test-report-nudge` (2026-07-31)** — redesigned by user direction away from this row's sketch: it parses no TRX/console output (the S6 refusal of `post-test-analyze` still holds); it nudges the model once per session to keep a plain-language `test-report.md` current, format user-approved. Group B conflict check (R5, five items) answered in CHANGELOG 0.3.44. Do not rebuild the parsing form. |
